...
public class KNNPredictor extends Predictor {
	...
	public ArrayList<DataPoint> trains = new ArrayList<DataPoint>();
		...
		try {
			...
			for(int j=0;j<fares.size()-1;j++) {
				...
				if(rand.nextDouble() <= 0.9) {
					...
					this.trains.add(new DataPoint(ages.get(j), fares.get(j), pClasses.get(j), sexes.get(j), labels.get(j), false));
	...
	public ArrayList<DataPoint> getTrains() {
		return this.trains;
	}
